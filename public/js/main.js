...
function getGravatarAvatar(email, size) {
  ...
  size = size || 64;
  return `https://www.gravatar.com/avatar/${MD5(email.trim())}?s=${size}`;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (user && !user.emailVerified) {
      user.sendEmailVerification()
        .then(() => {
          ...
          verify.innerHTML = "<p>Verification email send.</p>"
        })
        .catch(console.error);
    }
  });
    ...
    if (email.checkValidity()) {
      auth.sendPasswordResetEmail(email.value)
        .then(() => {
          ...
          document.getElementById("loginInputs").classList.add("hide");
          document.getElementById("btnReset").classList.add("hide");
          document.querySelector("#loginForm h2").innerHTML = "Password reset email send.";
        })
        .catch(console.error);
    } else {
  ...
  firebase.auth().onAuthStateChanged(user => {
    ...
    if (!user) {
      ...
      document.getElementById("loginInputs").classList.remove("hide");
      document.getElementById("avatar").classList.add("hide");
      document.querySelector("#loginForm h2").innerHTML = "Login or sign up";
    ...
    if (user) {
      ...
      document.querySelector("#avatar img").src = getGravatarAvatar(user.email, 128);
      document.querySelector("#loginForm h2").innerHTML = `Logged in as ${user.displayName ? user.displayName : user.email}`;
    }
    console.log(user);
    if (user && !user.emailVerified) {
      verify.classList.remove("hide");
